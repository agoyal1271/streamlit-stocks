...
def analyze_options_data(calls_df: pd.DataFrame, puts_df: pd.DataFrame, current_price: float) -> Dict:
    ...
    if not calls_df.empty and not puts_df.empty:
        ...
        for strike in sorted(set(calls_df['strike']).union(set(puts_df['strike']))):
            ...
            # Calls: positive gamma, Puts: negative gamma when long
            # Market makers are typically short options, so flip signs
        ...
        positive_gamma = gamma_df[gamma_df['net_gamma_exposure'] > 0]
        negative_gamma = gamma_df[gamma_df['net_gamma_exposure'] < 0]
        
        analysis['gamma_flip_level'] = None
        analysis['gamma_regime'] = 'Neutral'
        
        if not positive_gamma.empty and not negative_gamma.empty:
            # Find the strike where gamma flips
            current_gamma = gamma_df[gamma_df['strike'].apply(lambda x: abs(x - current_price)).idxmin()]
            if not current_gamma.empty:
                current_net_gamma = current_gamma.iloc[0]['net_gamma_exposure'] if hasattr(current_gamma.iloc[0], '__getitem__') else current_gamma['net_gamma_exposure']
        ...
        analysis['gamma_walls'] = {
            'resistance': gamma_df[gamma_df['price_distance_pct'] > 0].head(3)['strike'].tolist(),
            'support': gamma_df[gamma_df['price_distance_pct'] < 0].head(3)['strike'].tolist()
        }
        
        # Enhanced gamma insights
        total_gamma_exposure = gamma_df['net_gamma_exposure'].sum()
        gamma_concentration = gamma_df.head(5)['gamma_strength'].sum() / gamma_df['gamma_strength'].sum()
        ...
        if not nearby_gamma.empty:
            max_nearby_strike = nearby_gamma.loc[nearby_gamma['gamma_strength'].idxmax(), 'strike']

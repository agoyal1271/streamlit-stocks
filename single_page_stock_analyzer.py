...
class StockAnalyzer:
    ...
    def analyze_settlement_patterns(self, ticker: str) -> Dict:
        """Analyze historical options settlement patterns"""
        conn = sqlite3.connect(self.db_name)
        
        # Get historical options data for settlement analysis
        query = """
            SELECT expiry_date, strike, option_type, open_interest, data_date
            FROM options_data 
            WHERE ticker = ? 
            AND expiry_date < date('now')
            ORDER BY expiry_date DESC, data_date DESC
        """
        ...
        settlement_analysis = {
            'total_settled_contracts': 0,
            'settled_call_contracts': 0,
            'settled_put_contracts': 0,
            'settlement_dates': [],
            'avg_settlement_oi': 0,
            'settlement_insights': []
        }
        ...
        # Group by expiry to find final OI before settlement
        try:
            settled_expirations = df.groupby(['expiry_date', 'option_type']).agg({
                'open_interest': 'last',  # Last recorded OI before expiry
                'data_date': 'max'
            }).reset_index()
            
            total_calls = settled_expirations[settled_expirations['option_type'] == 'call']['open_interest'].sum()
            total_puts = settled_expirations[settled_expirations['option_type'] == 'put']['open_interest'].sum()
            
            settlement_analysis['total_settled_contracts'] = total_calls + total_puts
            settlement_analysis['settled_call_contracts'] = total_calls
            settlement_analysis['settled_put_contracts'] = total_puts
            settlement_analysis['settlement_dates'] = sorted(df['expiry_date'].unique())
            
            if len(settled_expirations) > 0:
                settlement_analysis['avg_settlement_oi'] = settled_expirations['open_interest'].mean()
            
            # Generate settlement insights
            if total_calls > 0 and total_puts > 0:
                settlement_ratio = total_puts / total_calls
                if settlement_ratio > 1.2:
                    settlement_analysis['settlement_insights'].append(
                        f"Historical settlement shows bearish bias - {settlement_ratio:.2f} put/call ratio"
                    )
                elif settlement_ratio < 0.8:
                    settlement_analysis['settlement_insights'].append(
                        f"Historical settlement shows bullish bias - {settlement_ratio:.2f} put/call ratio"
                    )
        except Exception as e:
            settlement_analysis['settlement_insights'].append(f"Settlement analysis error: {str(e)}")
...
if 'options_analysis' in st.session_state:
    ...
    if analysis.get('detailed_gamma') is not None and not analysis['detailed_gamma'].empty:
        ...
        with st.expander("📚 Understanding Gamma Effects", expanded=False):
            st.markdown("""
            **What is Gamma?**
            Gamma measures how much an option's delta changes when the stock price moves $1. 
            
            **Why Gamma Matters:**
            - **High Gamma = Price Magnets**: Strikes with lots of options create "magnetic" effects
            - **Market Maker Hedging**: When you buy options, dealers hedge by buying/selling stock
            - **Volatility Control**: Gamma positioning determines if moves get amplified or dampened
            
            **Reading the Analysis:**
            - **Positive Gamma Regime**: Dealers stabilize price (sell rallies, buy dips)
            - **Negative Gamma Regime**: Dealers amplify moves (buy rallies, sell dips)
            - **Gamma Walls**: Price levels with massive option concentration that resist breakouts
            """)
        
        # Gamma regime indicator with detailed explanation
        ...
        with col1:
            ...
            # Detailed regime explanation
            if "Positive" in analysis.get('gamma_regime', ''):
                st.info("**Price Stabilization Mode**: Market makers will sell into strength and buy weakness, creating price support/resistance.")
            elif "Negative" in analysis.get('gamma_regime', ''):
                st.warning("**Price Amplification Mode**: Market makers will buy into strength and sell into weakness, accelerating moves.")
            else:
                st.info("**Neutral Mode**: Limited gamma effects on price action.")
        ...
        with col2:
            if analysis.get('gamma_walls', {}).get('resistance'):
                ...
                st.caption(f"Heavy call activity creates selling pressure above ${nearest_resistance:.0f}")
            else:
                st.metric("Gamma Resistance", "None nearby")
                st.caption("No significant call walls detected nearby")
        ...
        with col3:
            if analysis.get('gamma_walls', {}).get('support'):
                ...
                st.caption(f"Heavy put activity creates buying support below ${nearest_support:.0f}")
            else:
                st.metric("Gamma Support", "None nearby")
                st.caption("No significant put walls detected nearby")
        
        # Enhanced gamma insights with market implications
        if analysis.get('gamma_insights'):
            st.subheader("🎯 Gamma Market Implications")
            
            # Categorize insights for better understanding
            regime_insights = [i for i in analysis['gamma_insights'] if 'gamma' in i.lower() and ('stabiliz' in i.lower() or 'amplif' in i.lower())]
            structure_insights = [i for i in analysis['gamma_insights'] if 'concentrat' in i.lower() or 'distribut' in i.lower() or 'pinning' in i.lower()]
            timing_insights = [i for i in analysis['gamma_insights'] if 'time' in i.lower() or 'expiry' in i.lower()]
            activity_insights = [i for i in analysis['gamma_insights'] if 'volume' in i.lower() or 'reposition' in i.lower() or 'magnet' in i.lower()]
            
            col1, col2 = st.columns(2)
            
            with col1:
                if regime_insights:
                    st.markdown("**Market Regime Effects:**")
                    for insight in regime_insights:
                        st.write(f"• {insight}")
                    
                    # Add practical trading implications
                    if "Positive" in analysis.get('gamma_regime', ''):
                        st.success("**Trading Implication**: Expect range-bound action. Breakouts likely to fail unless sustained volume.")
                    elif "Negative" in analysis.get('gamma_regime', ''):
                        st.error("**Trading Implication**: Expect momentum moves. Small breaks can cascade into larger moves.")
        ...
        # Detailed gamma table with explanations
        st.subheader("📊 Gamma Levels Breakdown")
        
        # Gamma summary metrics
        if 'detailed_gamma' in analysis:
            gamma_df = analysis['detailed_gamma']
            
            col1, col2, col3, col4 = st.columns(4)
            
            with col1:
                total_gamma_exposure = gamma_df['net_gamma_exposure'].sum()
                st.metric("Total Net Gamma", f"{total_gamma_exposure:,.0f}")
                st.caption("Positive = Stabilizing, Negative = Amplifying")
            
            with col2:
                atm_strikes = gamma_df[gamma_df['distance_from_current'] < 2.5]  # Within 2.5%
                atm_gamma = atm_strikes['net_gamma_exposure'].sum() if not atm_strikes.empty else 0
                st.metric("ATM Gamma", f"{atm_gamma:,.0f}")
                st.caption("Near-the-money gamma concentration")
            
            with col3:
                max_strike_idx = gamma_df['gamma_strength'].idxmax()
                max_strike = gamma_df.loc[max_strike_idx, 'strike'] if max_strike_idx in gamma_df.index else 0
                st.metric("Max Gamma Strike", f"${max_strike:.0f}")
                st.caption("Strike with highest gamma concentration")
            
            with col4:
                gamma_range = gamma_df[gamma_df['distance_from_current'] < 5]  # Within 5%
                range_concentration = len(gamma_range) / len(gamma_df) * 100 if len(gamma_df) > 0 else 0
                st.metric("Nearby Concentration", f"{range_concentration:.0f}%")
                st.caption("Percentage of gamma within 5% of price")
        
        # Enhanced gamma table
        st.markdown("**Key Gamma Levels (Top 8):**")
        ...
        gamma_display['Effect'] = gamma_display.apply(lambda row: 
            'Strong Magnet' if row['gamma_strength'] > gamma_display['gamma_strength'].quantile(0.8) else
            'Moderate Magnet' if row['gamma_strength'] > gamma_display['gamma_strength'].quantile(0.6) else
            'Weak Effect', axis=1)
        
        gamma_display['Position'] = gamma_display['distance_from_current'].apply(lambda x: 
            'Very Close' if x < 1 else 'Close' if x < 3 else 'Moderate' if x < 7 else 'Far')
        
        # Format for display
        gamma_display['Strike'] = gamma_display['strike'].apply(lambda x: f"${x:.0f}")
        gamma_display['Total OI'] = gamma_display['total_oi'].apply(lambda x: f"{x:,.0f}")
        gamma_display['Net Gamma'] = gamma_display['net_gamma_exposure'].apply(lambda x: f"{x:,.0f}")
        gamma_display['Distance'] = gamma_display['distance_from_current'].apply(lambda x: f"{x:.1f}%")
        gamma_display['Strength'] = gamma_display['gamma_strength'].apply(lambda x: f"{x:,.0f}")
        
        # Show table with explanations
        display_df = gamma_display[['Strike', 'Total OI', 'Net Gamma', 'Distance', 'Effect', 'Position']].head(8)
        st.dataframe(display_df, use_container_width=True)
        
        # Table interpretation
        st.caption("""
        **Table Guide**: 
        • **Net Gamma**: Positive values = price stabilization, Negative = amplification
        • **Effect**: How strong the magnetic effect is at this strike
        • **Position**: How close this level is to current price (closer = more immediate impact)
        """)
        
        # Practical trading summary
        st.subheader("📈 Practical Trading Summary")
        
        # Generate specific trading implications
        trading_summary = []
        
        if analysis.get('gamma_regime') == 'Positive Gamma (Stabilizing)':
            trading_summary.append("**Range Trading Favored**: Look for mean reversion strategies, sell breakouts")
            trading_summary.append("**Support/Resistance Strong**: Gamma levels likely to hold on first test")
        elif analysis.get('gamma_regime') == 'Negative Gamma (Amplifying)':
            trading_summary.append("**Momentum Trading Favored**: Look for breakout strategies, ride trends")
            trading_summary.append("**Support/Resistance Weak**: Gamma levels may fail on first test")
        
        # Add specific level guidance
        if analysis.get('gamma_walls', {}).get('resistance'):
            nearest_resistance = min(analysis['gamma_walls']['resistance'], key=lambda x: abs(x - st.session_state.current_price))
            if (nearest_resistance - st.session_state.current_price) / st.session_state.current_price * 100 < 5:
                trading_summary.append(f"**Immediate Resistance**: Heavy selling likely near ${nearest_resistance:.0f}")
        
        if analysis.get('gamma_walls', {}).get('support'):
            nearest_support = max(analysis['gamma_walls']['support'], key=lambda x: -abs(x - st.session_state.current_price))
            if (st.session_state.current_price - nearest_support) / st.session_state.current_price * 100 < 5:
                trading_summary.append(f"**Immediate Support**: Heavy buying likely near ${nearest_support:.0f}")
        
        # Display trading summary
        if trading_summary:
            for summary in trading_summary:
                st.info(summary)
        
        # Risk warning
        st.warning("**Risk Disclaimer**: Gamma analysis shows market structure but doesn't guarantee price direction. Always use proper risk management.")
